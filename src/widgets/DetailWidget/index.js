  ...
  return (
    <section className={`overflow-hidden bg-slate-50 pt-14  pb-12 2xl:pt-[120px] 2xl:pb-[100px]`} id="DetailWidget">
      <div className="container">
        <div className="2xl:px-24 mt-8 mb-8">
          <p className=" text-blue-600 text-base font-semibold  leading-none mb-2"><Link href={`/`} className="uppercase">{t('Home')}</Link> / <Link href={`/blogs`} className="uppercase">{t('Blogs')}</Link></p>
          <div className="grid grid-cols-12 lg:gap-x-5 gy-3 items-end">
            <div className="col-span-12 lg:col-span-9">
              <h1 className="  text-3xl lg:text-5xl xl:text-6xl font-semibold  lg:leading-normal xl:leading-tight bg-gradient-to-r from-[#242E49]  to-[#5B95F9] bg-clip-text text-transparent font-condensed mb-1 lg:mb-1">{blogData?.title}</h1>
              <div className="text-neutral-700 text-base">{blogData?.created_at && `Published on ${new Date(blogData?.created_at).toLocaleDateString('en-GB')}`}</div>
            </div>
        ...
        <div className="2xl:px-24 py-14">
          <div className="grid grid-cols-12 lg:gap-x-10">
            <div className="col-span-9"><div className="prose max-w-none" dangerouslySetInnerHTML={{ __html: getLocalizedBlog(blogLanguage,locale)?.long_description }}></div></div>
