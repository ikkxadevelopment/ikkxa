...
export default function DetailWidget({ datas }) {
...
const [blogCategories, setBlogCategories] = useState([])
  ...
  const categoryUrl = BLOG_CATEGORY
    ...
    <section className={`overflow-hidden bg-slate-50 pt-14  pb-12 2xl:pt-[120px] 2xl:pb-[100px]`} id="DetailWidget">
      <div className="container">
        <div className="2xl:px-24 mb-8">
          <p className=" text-blue-600 text-base font-semibold  leading-none mb-2"><Link href={`/`} className="uppercase">HOME</Link> / <Link href={`/blogs`} className="uppercase">Blogs</Link></p>
          <div className="grid grid-cols-12 lg:gap-x-5 gy-3 items-end">
            <div className="col-span-12 lg:col-span-9">
              ...
              <div className="text-neutral-700 text-base">{datas?.created_at && `Published on ${new Date(datas?.created_at).toLocaleDateString('en-GB')}`}</div>
            </div>
        ...
        {datas?.image && <Image src={`${datas?.image}`} className="object-cover" fill alt={datas?.title} />}
        </div>

        <div className="2xl:px-24 py-14">
          <div className="grid grid-cols-12 lg:gap-x-10">
            <div className="col-span-9"><div className="prose max-w-none" dangerouslySetInnerHTML={{ __html: datas?.long_description }}></div></div>
            <div className="col-span-3">
              {/* <div className="text-blue-600 text-2xl font-semibold mb-2">{t(`More${cat}`)}</div> */}
              {datas?.blogs?.map((item, i) => {
                return (
                    ...
                    <div className="grid grid-cols-12 gap-x-3 items-center">
                      <div className="col-span-4">
                        <div className="aspect-[1/1] relative">
                          <Image src={`${item?.thumbnail}`} className="object-cover" fill alt={datas?.title} />
                        </div>
                      </div>
