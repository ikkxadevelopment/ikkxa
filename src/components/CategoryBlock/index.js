...
export default function CategoryBlock({ data }) {
    return (
        <Link className="block" href={`/categories/${data?.slug}`}>
            <div className='aspect-1/1 relative bg-slate-50 overflow-hidden rounded-lg'>
                <Image src={data?.all_popular_image} fill className="object-cover" alt={`${data?.title} image`} />
            </div>
            <div className='pt-2'>
